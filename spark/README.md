...
* Spark 3.5.6 for Hadoop 3.2 with OpenJDK 8 and Scala 2.12
* Spark 3.5.6 for Hadoop 3.2 with OpenJDK 11 and Scala 2.12
...
services:
  spark-master:
    image: bde2020/spark-master:3.5.6-hadoop3.2
  ...
  spark-worker-1:
    image: bde2020/spark-worker:3.5.6-hadoop3.2
...
To start a Spark master:
    ...
    docker run --name spark-master -h spark-master -e ENABLE_INIT_DAEMON=false -d bde2020/spark-master:3.5.6-hadoop3.2

### Spark Worker
To start a Spark worker:

    docker run --name spark-worker-1 --link spark-master:spark-master -e ENABLE_INIT_DAEMON=false -d bde2020/spark-worker:3.5.6-hadoop3.2
...
`kubectl run spark-base --rm -it --labels="app=spark-client" --image bde2020/spark-base:3.5.6-hadoop3.2 -- bash ./spark/bin/spark-shell --master spark://spark-master:7077 --conf spark.driver.host=spark-client`

To use `spark-submit` issue for example

`kubectl run spark-base --rm -it --labels="app=spark-client" --image bde2020/spark-base:3.5.6-hadoop3.2 -- bash ./spark/bin/spark-submit --class CLASS_TO_RUN --master spark://spark-master:7077 --deploy-mode client --conf spark.driver.host=spark-client URL_TO_YOUR_APP`
